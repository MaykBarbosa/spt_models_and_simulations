...
D_1 = mu_t_df.loc[pd.to_datetime('2006-01-01'):pd.to_datetime('2008-01-01')].index
D_2 = mu_t_df.loc[pd.to_datetime('2008-01-01'):pd.to_datetime('2014-01-01')].index
D_3 = mu_t_df.loc[pd.to_datetime('2014-01-01'):pd.to_datetime('2018-01-01')].index
D_4 = mu_t_df.loc[pd.to_datetime('2017-12-31'):].index
...
class PINN(tf.keras.Model):
    def __init__(self, input_dim=100, **kwargs):
        ...
        self.hidden_layers = tf.keras.Sequential([
            ...
            tf.keras.layers.Dense(20, activation='tanh', kernel_initializer=self.initializer, bias_initializer=self.initializer_bias, kernel_regularizer=self.l2_regularizer),
...
def main(epochs, save_dir):
    ...
    for D_i in curriculum_data.keys():
        ...
        for epoch in range(1,epochs+1):
            ...
            total_loss, equation_error, positivity_penalty, g_t, pi_t, gradient_self_adp, grad_G = train_step(model, optimizer, mu_di, stock_ret_di, mkt_ret_di, tau_di)
            ...
            if epoch % 10 == 0 or epoch == 1:
                print(f"Epoch {epoch} | Batch: {D_i} | total_loss: {total_loss.numpy()[0,0]}, MasterEqErr: {equation_error.numpy()}, DriftPenlt:: {positivity_penalty.numpy()}, lambda1: {self_adp_lambda1.numpy()[0,0]}, lambda2: {self_adp_lambda2.numpy()[0,0]}")
