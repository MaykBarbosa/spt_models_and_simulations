...
def main():
    ...
    for epoch in range(epochs):
        ...
        mkt_weights_per_epoch[str(epoch)] =  pi_t.numpy().tolist()
        drift_per_batch_vect.append(g_t.numpy())
        avg_epoch_loss_vect.append(total_loss.numpy()[0,0])


        if epoch % 10 == 0:
            print(f"Testing error for Epoch {epoch}:  total_loss: {total_loss.numpy()[0,0]}, master eq error: {equation_error.numpy()}, drift_penalty: {positivity_penalty.numpy()}")

    return avg_epoch_loss_vect, grad_norms_per_epoch, mkt_weights_per_epoch, drift_per_epoch, model
