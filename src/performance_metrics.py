...
def plot_G_nn_with_test_highlight(model, mu_tf, R_t_tf, test_idx, dates, ax=None):
    ...
    inputs_all = mu_tf
    G_p_nn_all = model(inputs_all, training=False).numpy().flatten()
...
def calculate_model_portfolio_return(model, mu, R_t_df):
    ...
    if not isinstance(grad_log_G_mu, tf.Tensor):
        grad_log_G_mu = tf.convert_to_tensor(grad_log_G_mu, dtype=mu.dtype)
    grad_log_G_plus_1 = grad_log_G_mu + tf.ones_like(grad_log_G_mu)
    sum_terms = grad_log_G_plus_1 - tf.reduce_sum(mu * grad_log_G_mu, axis=1, keepdims=True)
    pi_t = sum_terms * mu
...
def plot_cumulative_portfolios_performance(series_dict, dates, target_vol=0.1):
    """
